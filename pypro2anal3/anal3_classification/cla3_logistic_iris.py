...
model = LogisticRegression(C=1.0, solver='lbfgs', max_iter=100,\
                           multi_class='auto', verbose=0)    # tuning parameter는 API 참조. 실무에선 중요하게 작용.print(model)
# solver='lbfgs' : l2 규제, softmax 지원, max_iter : 학습 회수,  multi_class : auto를 주면 알아서 처리
# C : 규제의 강도. 1.0이 기본값.,  verbose: 학습과정 보여주나 안보여주냐. 0일때 안보여줌
print(model)
